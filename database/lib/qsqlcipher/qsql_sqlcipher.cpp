...
{
public:
    QSQLCipherResultPrivate(QSQLCipherResult *res);
    void cleanup();
...
QSQLCipherResultPrivate::QSQLCipherResultPrivate(QSQLCipherResult* res) : q(res), access(0),
...
bool QSQLCipherResultPrivate::fetchNext(QSqlCachedResult::ValueCache &values, int idx, bool initialFetch)
{
    ...
    case SQLITE_ERROR:
        ...
        q->setLastError(qMakeError(access, QCoreApplication::translate("QSQLCipherResult",
...
QSQLCipherResult::QSQLCipherResult(const QSQLCipherDriver* db)
    : QSqlCachedResult(db)
{
    d = new QSQLCipherResultPrivate(this);
    d->access = db->d_func()->access;
    const_cast<QSQLCipherDriverPrivate*>(db->d_func())->results.append(this);
}
...
QSQLCipherResult::~QSQLCipherResult()
{
    ...
    if (sqlDriver)
        const_cast<QSQLCipherDriverPrivate*>(qobject_cast<const QSQLCipherDriver *>(sqlDriver)->d_func())->results.removeOne(this);
    d->cleanup();
    delete d;
}
...
bool QSQLCipherResult::prepare(const QString &query)
{
    ...
    if (res != SQLITE_OK) {
        setLastError(qMakeError(d->access, QCoreApplication::translate("QSQLCipherResult",
...
void QSQLCipherDriver::close()
{
    Q_D(QSQLCipherDriver);
    if (isOpen()) {
        foreach (QSQLCipherResult *result, d->results) {
            result->d->finalize();
        }
