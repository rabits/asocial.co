...
void Network::readUDP()
{
    while (m_udp_socket->hasPendingDatagrams()) {
        ...
        if( version > NETWORK_PROTOCOL_VERSION ) {
            qWarning() << "Skipping datagram. Version of protocol" << version << "is greater than supported" << NETWORK_PROTOCOL_VERSION;
            return;
        } else if( version < NETWORK_MINIMAL_PROTOCOL_VERSION ) {
            qWarning() << "Skipping datagram. Version of protocol" << version << "is lower than supported" << NETWORK_MINIMAL_PROTOCOL_VERSION;
            return;
        }
